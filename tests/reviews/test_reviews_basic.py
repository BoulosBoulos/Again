...
def test_rating_out_of_range_fails():
    ...
    res_low = client.post("/reviews", json=body_low, headers=headers)
    assert res_low.status_code == 422

    # rating too high
    body_high = {"room_id": 1, "rating": 6, "comment": "Too high"}
    res_high = client.post("/reviews", json=body_high, headers=headers)
...
def test_moderator_cannot_create_review():
    ...
    res = client.post("/reviews", json=body, headers=headers_mod)
...
def test_auditor_and_service_account_cannot_flag_reviews():
    ...
    res_aud_flag = client.post(f"/reviews/{review_id}/flag", headers=headers_aud)
    res_svc_flag = client.post(f"/reviews/{review_id}/flag", headers=headers_svc)
...
def test_admin_can_update_and_delete_any_review():
    ...
    res_delete = client.delete(f"/reviews/{review_id}", headers=headers_admin)
...
def test_list_all_reviews_user_only_flagged_and_include_hidden_filters():
    ...
    res_flag = client.post(f"/reviews/{review2_id}/flag", headers=headers_admin)
    assert res_flag.status_code == 200
    ...
    res_user1 = client.get(
        "/reviews", params={"user_id": 1}, headers=headers_admin
    )
    ...
    res_flagged = client.get(
        "/reviews", params={"only_flagged": True}, headers=headers_admin
    )
    ...
    res_visible = client.get(
        "/reviews", params={"include_hidden": False}, headers=headers_admin
    )
